...
module pcb_button(length, width, height, button_length, button_width, button_height) {
  module base() {
    color("#ddd") cube([length, width, height]);
  }

  module button() {
    color("#111") translate([
        (length - button_length) / 2,
        (width - button_width) / 2,
        height]) cube([button_length, button_width, button_height]);
  }
